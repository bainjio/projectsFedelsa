  ...
  function FooPicker() {
    ...
    _self.showPicker = function() {
      ...
      var pickerField = document.getElementById(_id);
			//console.log(pickerField.getBoundingClientRect());
      ...
      if (pickerField) {
        ...
        var top = pickerField.offsetTop;
        if (pickerDiv) {
          pickerDiv.style.position = 'absolute';
          pickerDiv.style.top = '850px';
...
var inputChekInPick = new FooPicker({
		id: 'checkin',
		dateFormat: 'yyyy-MM-dd'
	});
	var inputChekOutPick = new FooPicker({
		id: 'checkout',
		dateFormat: 'yyyy-MM-dd'
	});

	// Function to redirect to the booking site, based on the selection 
	// of one of the two hostels and pre-populate the checkin, checkout, adults and children fields
	function redirect() {
		var hostel = document.getElementById("inputHostel").options[document.getElementById("inputHostel").selectedIndex].value;
		...
		var url = "";
		var urlAppend = "checkin=" + checkinDate + "&checkout=" + checkoutDate + "&adults=" + adults + "&children=" + children;

		if (hostel == 1) {
			var url = "https://hotels.cloudbeds.com/reservation/uA6xsY#" + urlAppend;
		}
		else if (hostel == 2) {
			var url = "https://hotels.cloudbeds.com/reservation/HQBVQU#" + urlAppend;
		}
		else {
			alert("Please select a hostel to continue.");
			return;
		}
